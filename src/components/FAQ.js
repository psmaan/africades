...
const FAQ = () => {
    ...
    return (
        <div className="w-screen bg-red-400 flex flex-col items-center py-12 px-4">
            <h2 className=" font-semibold mb-4 text-8xl">FAQ</h2>
            <div className="w-full max-w-4xl">
                {faqs.map((faq, index) => (
                    <div key={index} className="mb-4">
                        <div
                            className={`p-4 bg-blue-500 text-white font-semibold cursor-pointer transition-all duration-300 flex justify-between items-center ${activeIndex === index ? 'rounded-t-lg' : 'rounded-lg'}`}
                        ...
                        <div
                            className={`overflow-hidden transition-all duration-300 bg-blue-500 text-white ${activeIndex === index ? 'max-h-96 p-4 rounded-b-lg' : 'max-h-0'}`}
                        >
                            {faq.answer}
                        </div>
                    </div>
